...
describe('Produit', () => {
  ...
  it('devrait échouer si le nom est manquant ou trop long', () => {
    ...
    expect(() => new Produit('Papier avec un nom très très très long qui dépasse la limite de 100 caractères', 'Lisse', 80, 5.99, 'Blanc')).toThrowError(/nom est obligatoire/);
  });
  ...
  it('devrait échouer si le prix est en dehors de la plage valide', () => {
    expect(() => new Produit('Papier A4', 'Lisse', 80, -5.99, 'Blanc')).toThrowError(/prix doit être un nombre entre 0 et 1000/);
    expect(() => new Produit('Papier A4', 'Lisse', 80, 1500, 'Blanc')).toThrowError(/prix doit être un nombre entre 0 et 1000/);
  });
